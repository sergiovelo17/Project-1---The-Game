class Enemy extends Person {
  ...
  wallInFront(user,theGame){
    if(this.currDirection == 'E'){
      for(let i = this.x; i < user.x-1; i++){
        if(theGame.theCanvas.detectLine(i, this.y)){
          return true;
        }
      }
    ...
    }else if(this.currDirection == 'N'){
      for(let i = this.y; i > user.y+1; i--){
        if(theGame.theCanvas.detectLine(this.x, i)){
          return true;
        }
    }
  }else if(this.currDirection == 'S'){
    for(let i = this.y; i < user.y-5; i++){
      if(theGame.theCanvas.detectLine(this.x, i)){
        return true;
      }
  }
